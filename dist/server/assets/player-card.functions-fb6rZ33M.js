import { c as createServerRpc, s as supabaseAdmin } from "./client.server-BhqJA0Kf.js";
import { l as createServerFn } from "./server-Dc2-qbCG.js";
import { s as stringType, e as enumType, o as objectType, c as booleanType, r as recordType, u as unknownType, n as numberType, d as requireSupabaseAuth } from "./auth-middleware-DNUs_ZGg.js";
import { a as INVENTORY_FLAGS, I as INVENTORY_CLUBS, c as getClubsForSport, A as AVATAR_INVENTORY, d as getFlagById, b as getClubById } from "./catalog-Du0eftkI.js";
import { p as parseSkillLevel } from "./player-card.utils-BjTbZwr2.js";
...
const getMyPlayerDashboard = createServerFn({
  ...
  const {
    data: matches
  } = await supabaseAdmin.from("player_match_history").select("*").eq("player_user_id", context.userId).order("match_date", {
    ascending: false
  }).limit(20);
...
const getPublicPlayerProfile = createServerFn({
  ...
  const {
    data: matches
  } = await supabaseAdmin.from("player_match_history").select("*").eq("player_user_id", profile.id).order("match_date", {
    ascending: false
  }).limit(12);
  return {
