...
import { o as objectType, s as stringType, n as numberType, d as requireSupabaseAuth, e as enumType, c as booleanType, b as arrayType } from "./auth-middleware-CXeE-dWU.js";
import { l as loadVenuePricing, c as calculateBookingTotal, r as resolvePayableAmount } from "./pricing-DOPRXSDA.js";
import { b as buildVenueDaySessions, M as MIN_ORDER_PAISE, d as syncGroupSlotPostsForBooking } from "./checkout-C9SCQ6tN.js";
import { i as isRazorpayConfigured, c as createRazorpayOrder } from "./razorpay-DwVM9bks.js";
import { n as slotStepMinutes, o as venueOpenMinutes, v as venueCloseMinutes, d as bookingStartMinute, a as bookingEndMinute, m as iterateBookingMinutes, k as isMinuteBlocked, g as formatMinBookingDuration, i as formatSlotTime } from "./slot-time-C7HRn-Jw.js";
import { l as listVenueMediaBySlug } from "./mysql.server-CjXhDLfO.js";
import { r as resolveMinBookingMinutes } from "./venue-extras-ZOJE-7Nv.js";
...
import "node:buffer";
import "events";
import "./worker-entry-2rahOB7i.js";
import "node:events";
import "net";
import "tls";
import "timers";
...
function isActiveHoldBooking(b) {
  if (b.status === "confirmed") return true;
  if (b.status !== "pending") return false;
  if (!b.created_at) return true;
  const ageMs = Date.now() - new Date(b.created_at).getTime();
  return ageMs < PENDING_HOLD_MINUTES * 60 * 1e3;
}
async function expireStalePendingBookings(venueId, date) {
  const cutoff = new Date(Date.now() - PENDING_HOLD_MINUTES * 60 * 1e3).toISOString();
  const {
    data: stale
  } = await supabaseAdmin.from("bookings").select("id, payment_id").eq("venue_id", venueId).eq("booking_date", date).eq("status", "pending").lt("created_at", cutoff);
  if (!stale?.length) return;
  const ids = stale.map((b) => b.id);
  await supabaseAdmin.from("bookings").update({
    status: "cancelled"
  }).in("id", ids);
  const paymentIds = stale.map((b) => b.payment_id).filter(Boolean);
...
let venueDetailExtrasReady = null;
let bookingMinuteColumnsReady = null;
let shareToGroupColumnReady = null;
...
async function venueHasShareToGroupColumn() {
  ...
  const {
    error
  } = await supabaseAdmin.from("bookings").select("share_to_group").limit(1);
...
const getVenue = createServerFn({
  ...
  const normalized = withReviewCount(venue);
...
const getSlots = createServerFn({
  ...
  const bookingFields = await venueHasBookingMinuteColumns() ? "id, start_hour, end_hour, start_minute, end_minute, status, player_count, is_open_lobby, created_at" : "id, start_hour, end_hour, status, player_count, is_open_lobby, created_at";
...
const getVenueDaySchedule_createServerFn_handler = createServerRpc({
  id: "f26a2403204432666753550337f3622ea83d6beb98fcb5538a190a3482b32146",
  name: "getVenueDaySchedule",
  filename: "src/lib/booking.functions.ts"
}, (opts) => getVenueDaySchedule.__executeServer(opts));
const getVenueDaySchedule = createServerFn({
  method: "GET"
}).inputValidator((input) => objectType({
  venueId: stringType().uuid(),
  date: stringType().regex(/^\d{4}-\d{2}-\d{2}$/)
}).parse(input)).handler(getVenueDaySchedule_createServerFn_handler, async ({
  data
}) => {
  const {
    data: venue
  } = await supabaseAdmin.from("venues").select("max_players_allowed, slot_duration_minutes, operating_days, holiday_dates").eq("id", data.venueId).maybeSingle();
  if (!venue) return [];
  const dow = (/* @__PURE__ */ new Date(data.date + "T12:00:00")).getDay();
  const holidays = venue.holiday_dates ?? [];
  if (holidays.includes(data.date)) return [];
  ...
  const {
    data: bookings
  } = await supabaseAdmin.from("bookings").select(bookingFields).eq("venue_id", data.venueId).eq("booking_date", data.date).in("status", ["confirmed", "pending"]);
  const stepMinutes = slotStepMinutes(venue.slot_duration_minutes);
  const totalCapacity = Math.max(1, Number(venue.max_players_allowed ?? 1));
  return buildVenueDaySessions((bookings ?? []).filter(isActiveHoldBooking), totalCapacity, stepMinutes);
});
...
const createBooking = createServerFn({
  ...
  const {
    data: overlaps,
    error: overlapErr
  } = await supabaseAdmin.from("bookings").select(await venueHasBookingMinuteColumns() ? "start_hour, end_hour, start_minute, end_minute, player_count, status, created_at" : "start_hour, end_hour, player_count, status, created_at").eq("venue_id", data.venueId).eq("booking_date", data.date).in("status", ["confirmed", "pending"]);
...
const cancelPendingBooking_createServerFn_handler = createServerRpc({
  id: "7c14dfcc113cd3bbf268768cc70e69951ff9ee49e3d842564fc390a32a59e7ea",
  name: "cancelPendingBooking",
  filename: "src/lib/booking.functions.ts"
}, (opts) => cancelPendingBooking.__executeServer(opts));
const cancelPendingBooking = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({
  bookingId: stringType().uuid()
}).parse(input)).handler(cancelPendingBooking_createServerFn_handler, async ({
  context,
  data
}) => {
  const {
    data: booking,
    error
  } = await supabaseAdmin.from("bookings").select("id, status, payment_id, user_id").eq("id", data.bookingId).eq("user_id", context.userId).maybeSingle();
  if (error) throw new Error(error.message);
...
const previewBookingTotal_createServerFn_handler = createServerRpc({
  id: "e6e5f6e307c3410f8bd20b479e303baf289c04ab7a6749ee4bd1882e906434cc",
  name: "previewBookingTotal",
  filename: "src/lib/booking.functions.ts"
}, (opts) => previewBookingTotal.__executeServer(opts));
const previewBookingTotal = createServerFn({
  ...
  message: "endMinute must be > startMinute"
}).parse(input)).handler(previewBookingTotal_createServerFn_handler, async ({
  data
}) => {
  const {
    data: venue,
    error
  } = await supabaseAdmin.from("venues").select("price_per_hour, max_players_allowed, slot_duration_minutes").eq("id", data.venueId).eq("is_active", true).maybeSingle();
...
export {
  cancelPendingBooking_createServerFn_handler,
  createBooking_createServerFn_handler,
  getSlots_createServerFn_handler,
  getVenueDaySchedule_createServerFn_handler,
  ...
  previewBookingTotal_createServerFn_handler
};
