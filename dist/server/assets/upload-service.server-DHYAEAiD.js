...
import { z as getAugmentedNamespace, A as getDefaultExportFromCjs } from "./server-Cnaa2j9o.js";
import require$$0$2 from "node:buffer";
import require$$0$5 from "events";
import { p as process$1 } from "./worker-entry-C-hOLRys.js";
...
const require$$0$1 = /* @__PURE__ */ getAugmentedNamespace(process$1);
var denque;
...
function requireClient() {
  ...
  client.CLIENT_QUERY_ATTRIBUTES = 134217728;
...
var encode_parameter;
var hasRequiredEncode_parameter;
function requireEncode_parameter() {
  if (hasRequiredEncode_parameter) return encode_parameter;
  hasRequiredEncode_parameter = 1;
  const Types = requireTypes();
  const Packet = requirePacket();
  ...
  encode_parameter = { toParameter, isJSON };
...
function requireExecute$1() {
  ...
  const Packet = requirePacket();
  const CharsetToEncoding = requireCharset_encodings();
  const { toParameter } = requireEncode_parameter();
  class Execute {
    ...
    _serializeToBuffer(buffer) {
      ...
      packet2.writeInt8(cursorFlags);
      packet2.writeInt32(1);
      if (useQueryAttributes) {
        packet2.writeLengthCodedNumber(totalParams);
      }
      if (totalParams > 0) {
        ...
        for (let i = 0; i < allParams.length; i++) {
          packet2.writeInt8(allParams[i].type);
          packet2.writeInt8(0);
          if (useQueryAttributes) {
            const name = i < numParams ? "" : attrNames[i - numParams];
            packet2.writeLengthCodedString(name, this.encoding);
          }
        }
        allParams.forEach((parameter) => {
          if (parameter.type !== Types.NULL) {
...
function requireHandshake_response() {
  ...
  class HandshakeResponse {
    constructor(handshake2) {
      ...
      if (handshake2.authToken !== void 0 && handshake2.authPluginName !== void 0) {
        if (!Buffer.isBuffer(handshake2.authToken)) {
          throw new TypeError(
            "HandshakeResponse authToken must be a Buffer when provided"
          );
        }
        if (typeof handshake2.authPluginName !== "string") {
          throw new TypeError(
            "HandshakeResponse authPluginName must be a string when provided"
          );
        }
        this.authToken = handshake2.authToken;
        this.authPluginName = handshake2.authPluginName;
      } else {
        let authToken;
        if (this.passwordSha1) {
          authToken = auth41.calculateTokenFromPasswordSha(
    ...
    serializeResponse(buffer) {
      ...
      if (isSet("PLUGIN_AUTH")) {
        packet2.writeNullTerminatedString(
          this.authPluginName || "mysql_native_password",
          "latin1"
        );
      }
    ...
    static fromPacket(packet2, serverFlags = 4294967295) {
      const args = {};
  ...
  class Query {
    ...
    serializeToBuffer(buffer) {
      ...
      if (useQueryAttributes) {
        ...
        packet2.writeLengthCodedNumber(paramCount);
        packet2.writeLengthCodedNumber(1);
        if (paramCount > 0) {
          ...
          for (let i = 0; i < paramCount; i++) {
            packet2.writeInt8(parameters[i].type);
            packet2.writeInt8(0);
            packet2.writeLengthCodedString(names[i], this.encoding);
          }
          parameters.forEach((parameter) => {
            if (parameter.type !== Types.NULL) {
              parameter.writer.call(packet2, parameter.value);
...
var reset_connection$1;
var hasRequiredReset_connection$1;
function requireReset_connection$1() {
  if (hasRequiredReset_connection$1) return reset_connection$1;
  hasRequiredReset_connection$1 = 1;
  const Packet = requirePacket();
  const CommandCodes = requireCommands$1();
  class ResetConnection {
    ...
    toPacket() {
      const packet2 = new Packet(0, Buffer.alloc(5), 0, 5);
      packet2.offset = 4;
      packet2.writeInt8(CommandCodes.RESET_CONNECTION);
      return packet2;
    }
  }
  reset_connection$1 = ResetConnection;
  return reset_connection$1;
}
var server_status = {};
...
function requirePackets() {
  ...
  (function(module, exports) {
    ...
    const ResetConnection = requireReset_connection$1();
...
var caching_sha2_password = { exports: {} };
var hasRequiredCaching_sha2_password;
function requireCaching_sha2_password() {
  if (hasRequiredCaching_sha2_password) return caching_sha2_password.exports;
  ...
  const pluginFactory = (pluginOptions = {}) => ({ connection: connection2 }) => {
    let state = 0;
    let scramble = null;
    const password = connection2.config.password;
    const authWithKey = (serverKey) => {
  ...
  caching_sha2_password.exports = pluginFactory;
  caching_sha2_password.exports.calculateToken = calculateToken;
  return caching_sha2_password.exports;
}
...
function requireAuth_switch() {
  ...
  const standardAuthPlugins = Object.assign(/* @__PURE__ */ Object.create(null), {
  ...
  function getAuthPlugin(pluginName, connection2) {
    const customPlugins = connection2.config.authPlugins;
  ...
  function authSwitchRequest(packet2, connection2, command2) {
    ...
    if (pluginName === "mysql_clear_password") {
      const hasCustomPlugin = connection2.config.authPlugins && Object.prototype.hasOwnProperty.call(
        connection2.config.authPlugins,
        "mysql_clear_password"
      );
      if (!hasCustomPlugin && !connection2.config.enableCleartextPlugin) {
        const err = new Error(
    ...
    const authPlugin = getAuthPlugin(pluginName, connection2);
    if (!authPlugin) {
      throw new Error(
...
function requireCompressed_protocol() {
  ...
  function enableCompression(connection2) {
    ...
    connection2.inflateQueue = new Queue();
    connection2.deflateQueue = new Queue();
  }
  compressed_protocol = {
    enableCompression,
    Queue
...
function requireClient_handshake() {
  ...
  const CharsetToEncoding = requireCharset_encodings();
  const auth41 = requireAuth_41();
  const { getAuthPlugin } = requireAuth_switch();
  const {
    calculateToken: calculateSha2Token
  } = requireCaching_sha2_password();
  ...
  class ClientHandshake extends Command {
    ...
    sendCredentials(connection2) {
      ...
      const isSecureConnection = connection2.config.ssl || connection2.config.socketPath;
      ...
      const hasCustomAuthPlugin = connection2.config.authPlugins && Object.prototype.hasOwnProperty.call(
        connection2.config.authPlugins,
        serverAuthMethod
      );
      const hasLegacyAuthSwitchHandler = typeof connection2.config.authSwitchHandler === "function";
      const canUseDirectAuth = !hasCustomAuthPlugin && !hasLegacyAuthSwitchHandler && this.canUseAuthMethodDirectly(serverAuthMethod, isSecureConnection) && (serverAuthMethod !== "mysql_clear_password" || connection2.config.enableCleartextPlugin);
      ...
      if (clientAuthMethod !== "mysql_native_password") {
        this.initializeAuthPlugin(clientAuthMethod, authPluginData, connection2);
      }
    }
    ...
    initializeAuthPlugin(authMethod, authPluginData, connection2) {
      const authPlugin = getAuthPlugin(authMethod, connection2);
      if (!authPlugin) {
        return;
      }
      const pluginHandler = authPlugin({ connection: connection2, command: this });
      connection2._authPlugin = pluginHandler;
      try {
...
function requireText_parser() {
  ...
  function readCodeFor(type, charset, encodingExpr, config, options) {
    ...
    switch (type) {
      ...
      case Types.DATE:
        if (helpers2.typeMatch(type, dateStrings, Types)) {
          return 'packet.readLengthCodedString("ascii")';
        }
...
function requireQuery() {
  ...
  class Query extends Command {
    ...
    start(_packet, connection2) {
      ...
      this._setTimeout();
      const clientFlags = connection2.config.clientFlags & (connection2.serverCapabilityFlags || 0);
...
var reset_connection;
var hasRequiredReset_connection;
function requireReset_connection() {
  if (hasRequiredReset_connection) return reset_connection;
  hasRequiredReset_connection = 1;
  const Command = requireCommand();
  const Packets = requirePackets();
  class ResetConnection extends Command {
    ...
    start(packet2, connection2) {
      ...
      connection2.writePacket(req.toPacket());
      return ResetConnection.prototype.resetConnectionResponse;
    }
    resetConnectionResponse(packet2, connection2) {
      if (connection2._statements) {
        connection2._statements.clear();
      }
      if (this.onResult) {
...
function requireCommands() {
  ...
  const ResetConnection = requireReset_connection();
...
function requireTracing() {
  ...
  const queryChannel = hasTracingChannel ? dc.tracingChannel("mysql2:query") : void 0;
  const executeChannel = hasTracingChannel ? dc.tracingChannel("mysql2:execute") : void 0;
  const connectChannel = hasTracingChannel ? dc.tracingChannel("mysql2:connect") : void 0;
  const poolConnectChannel = hasTracingChannel ? dc.tracingChannel("mysql2:pool:connect") : void 0;
...
var capture_local_err;
var hasRequiredCapture_local_err;
function requireCapture_local_err() {
  if (hasRequiredCapture_local_err) return capture_local_err;
  hasRequiredCapture_local_err = 1;
  ...
  capture_local_err = { captureStackHolder, applyCapturedStack };
  return capture_local_err;
}
var make_done_cb;
...
function requireMake_done_cb() {
  ...
  const { applyCapturedStack } = requireCapture_local_err();
...
function requirePrepared_statement_info() {
  ...
  const { captureStackHolder } = requireCapture_local_err();
...
function requirePool$2() {
  ...
  class BasePool extends EventEmitter {
      ...
      const _getConnection = (cb2) => {
        ...
        if (this._freeConnections.length > 0) {
          ...
          return process2.nextTick(() => {
            connection2._released = false;
            cb2(null, connection2);
          });
        }
    ...
    releaseConnection(connection2) {
      ...
      if (this.config.resetOnRelease && connection2.reset) {
        connection2.reset((err) => {
          if (err) {
            connection2._pool = null;
          ...
          this._handleSuccessfulRelease(connection2);
        });
      } else {
        this._handleSuccessfulRelease(connection2);
      }
    }
    _handleSuccessfulRelease(connection2) {
      let cb;
      if (this._connectionQueue.length) {
        cb = this._connectionQueue.shift();
        process2.nextTick(() => {
...
function hasUserMediaConfig() {
  return Boolean(
    process.env.USER_MEDIA_DATABASE_URL || process.env.MEDIA_DATABASE_URL || process.env.MYSQL_HOST && process.env.MYSQL_USER && process.env.MYSQL_PASSWORD && process.env.MYSQL_DATABASE
  );
}
function hasVenueMediaConfig() {
  return Boolean(
    process.env.VENUE_MEDIA_DATABASE_URL || process.env.VENUE_MEDIA_HOST && process.env.VENUE_MEDIA_USER && process.env.VENUE_MEDIA_PASSWORD && process.env.VENUE_MEDIA_DATABASE
  );
}
function isMysqlMediaEnabled() {
  return hasUserMediaConfig() || hasVenueMediaConfig();
}
function mediaScopeForCategory(category) {
  return category === "avatars" ? "user" : "venue";
}
function getMediaPublicPath(scope, assetId) {
  return `/api/media/${scope}/${assetId}`;
}
function getMediaPublicUrl$1(scope, assetId) {
  const path2 = getMediaPublicPath(scope, assetId);
  const base = process.env.MEDIA_PUBLIC_URL?.replace(/\/$/, "");
  return base ? `${base}${path2}` : path2;
}
function getUserMediaPool() {
...
async function saveMediaToMysql(opts) {
  ...
  const path2 = getMediaPublicPath(scope, id);
  return { id, scope, path: path2, url: getMediaPublicUrl$1(scope, id) };
}
