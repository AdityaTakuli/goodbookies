/**
 * Upload Yorker Yard Rectangular turf media (3 photos) and link on the venue.
 *
 * Usage (from repo root):
 *   set -a && source .env && set +a && node scripts/upload-yorker-yard-rectangular-media.mjs
 */
...
const VENUE_SLUG = "yorker-yard-rectangular";
const PUBLIC_DIR = path.join(ROOT, "public", "venues", "yorker-yard-rectangular");

const ASSETS = [
  {
    source: "Turf_Media/Yorker_Yard_Rectangular/TopView.png",
  ...
  {
    source: "Turf_Media/Yorker_Yard_Rectangular/SideView.png",
  ...
  {
    source: "Turf_Media/Yorker_Yard_Rectangular/Inside.png",
    publicName: "inside.png",
    category: "venues",
    mime: "image/png",
    type: "image",
    label: "Inside view",
...
function saveToPublic(asset, buffer) {
  ...
  return `/venues/yorker-yard-rectangular/${asset.publicName}`;
}
...
async function main() {
  ...
  console.log("\nDone — Yorker Yard Rectangular media linked:");
